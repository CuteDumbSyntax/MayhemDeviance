...
namespace MayhemDeviance.Common.Systems
{
	...
	public class ModIntegrationsSystem : ModSystem
	{
		...
		private void DoBossChecklistIntegration() {
			...
			string ArgunName = "Argun";
			string MAXName = "MayhemCat";
			string CIName = "IlyasClone";
			...
			Func<bool> downed = () => DownedBossSystem.ArgunBoss;
			Func<bool> Mdowned = () => DownedBossSystem.MaxBoss;
			Func<bool> CIdowned = () => DownedBossSystem.CIBoss;

			// The NPC type of the boss
			int ArgunB = ModContent.NPCType<Content.Bosses.Argun>();
			int MaxB = ModContent.NPCType<Content.Bosses.MayhemCat>();
			int CIB = ModContent.NPCType<Content.Bosses.IlyasClone>();

			// The item used to summon the boss with (if available)
			int ArgunS = ModContent.ItemType<Content.Items.Consumables.BadApple>();
			int MaxS = ModContent.ItemType<Content.Items.Consumables.PossesedHeart>();
			int CIS = ModContent.ItemType<Content.Items.Consumables.PossesedHeart>();

			// "collectibles" like relic, trophy, mask, pet
			List<int> ArgunCollectibles = new List<int>()
			{
				ModContent.ItemType<Content.Items.Placeable.Furniture.ArgunRelic>(),
			...
			List<int> MaxCollectibles = new List<int>()
			{
				ModContent.ItemType<Content.Items.Placeable.Furniture.MayhemCatRelic>(),
				// ModContent.ItemType<Content.Pets.MinionBossPet.MinionBossPetItem>(),
				// ModContent.ItemType<Content.Items.Placeable.Furniture.MinionBossTrophy>(),
				// ModContent.ItemType<Content.Items.Armor.Vanity.MinionBossMask>()
			};

			List<int> CICollectibles = new List<int>()
			{
				ModContent.ItemType<Content.Items.Placeable.Furniture.MayhemCatRelic>(),
				// ModContent.ItemType<Content.Pets.MinionBossPet.MinionBossPetItem>(),
				// ModContent.ItemType<Content.Items.Placeable.Furniture.MinionBossTrophy>(),
			...
			var customPortrait = (SpriteBatch sb, Rectangle rect, Color color) => {
				Texture2D texture = ModContent.Request<Texture2D>("MayhemDeviance/Assets/Textures/Bestinary/Argun_Preview").Value;
				Vector2 centered = new Vector2(rect.X + (rect.Width / 2) - (texture.Width / 2), rect.Y + (rect.Height / 2) - (texture.Height / 2));
				sb.Draw(texture, centered, color);
			};

			var MaxPortrait = (SpriteBatch sb, Rectangle rect, Color color) => {
				Texture2D texture = ModContent.Request<Texture2D>("MayhemDeviance/Assets/Textures/Bestinary/MayhemCat_Preview").Value;
				Vector2 centered = new Vector2(rect.X + (rect.Width / 2) - (texture.Width / 2), rect.Y + (rect.Height / 2) - (texture.Height / 2));
				sb.Draw(texture, centered, color);
			};

			var CIPortrait = (SpriteBatch sb, Rectangle rect, Color color) => {
				Texture2D texture = ModContent.Request<Texture2D>("MayhemDeviance/Assets/Textures/Bestinary/IlyasClone_Preview").Value;
				Vector2 centered = new Vector2(rect.X + (rect.Width / 2) - (texture.Width / 2), rect.Y + (rect.Height / 2) - (texture.Height / 2));
				sb.Draw(texture, centered, color);
			};
			...
			bossChecklistMod.Call(
				...
				new Dictionary<string, object>() {
					["spawnItems"] = ArgunS,
					["collectibles"] = ArgunCollectibles,
			...
			bossChecklistMod.Call(
				...
				new Dictionary<string, object>() {
					["spawnItems"] = MaxS,
					["collectibles"] = MaxCollectibles,
					["customPortrait"] = MaxPortrait
					// Other optional arguments as needed are inferred from the wiki
				}
			);
			bossChecklistMod.Call(
				...
				new Dictionary<string, object>() {
					["spawnItems"] = CIS,
					["collectibles"] = CICollectibles,
					["customPortrait"] = CIPortrait
					// Other optional arguments as needed are inferred from the wiki
				}
			);
